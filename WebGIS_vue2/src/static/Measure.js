...
export class Measure {
   ...
   */
  constructor(arg) {
    if (arg.viewer && arg.viewer instanceof Cesium.Viewer) {
      this._drawLayer = new Cesium.CustomDataSource("measureLayer");
      this.objArr = {};
      arg.viewer && arg.viewer.dataSources.add(this._drawLayer);
      this._basePath = arg.basePath || "";
      this._viewer = arg.viewer;
    } else {
      throw new Error("there is no Cesium.Viewer");
    }
  }
  ...
  drawLineMeasureGraphics(options = {}) {
    if (this._viewer && options) {
      var positions = [],
        ...
        _handlers = this.get_Handler($this._viewer);
      $this.objArr[objId] = new Cesium.CustomDataSource(objId);
      $this._viewer.dataSources.add($this.objArr[objId]);
      // left
      ...
      _lineEntity.polyline = {
        ...
        material: options.material || Cesium.Color.fromRandom({
          minimumRed: 0.0,
          minimumGreen: 0.0,
          minimumBlue: 0.0,
          maximumRed: 1.0,
      ...
      $this.objArr[objId].entities.add(_lineEntity);
      //添加固定点坐标点
      function _addInfoPoint(position) {
        ...
        $this.objArr[objId].entities.add(_labelEntity);
      }
    }
  }
  /**
  ...
  drawAreaMeasureGraphics(options = {}) {
    if (this._viewer && options) {
      ...
      var turfarea = '',
        ...
        polyObj = null,
        _label = "",
        _handler = this.get_Handler($this._viewer);
      let objId = Number(
        new Date().getTime() + "" + Number(Math.random() * 1000).toFixed(0)
      );
      $this.objArr[objId] = new Cesium.CustomDataSource(objId);
      ...
      _handler.setInputAction(function (movement) {
        var cartesian = $this.getCatesian3FromPX(movement.position);
        let vcartesian = Cesium.SceneTransforms.wgs84ToWindowCoordinates($this._viewer.scene,cartesian);
        if (cartesian && cartesian.x) {
          if (positions.length == 0) {
        ...
        if (vcartesian && vcartesian.x) {
          vpositions.push(vcartesian.clone());
      ...
      // right
      _handler.setInputAction(function (movement) {
        let cur_cartesian = $this.getCatesian3FromPX(movement.position);
        let v_cartesian = Cesium.SceneTransforms.wgs84ToWindowCoordinates($this._viewer.scene,cur_cartesian);
        _handler.destroy();
        // positions.pop();
        // positions.push(cur_cartesian);
        positions.push(positions[0]);
        vpositions.push(v_cartesian);
        vpositions.push(vpositions[0]);
        mpositions.push(cur_cartesian);
        mpositions.push(positions[0]);
        // for(let i = 0;i<positions.length;i++){
        //   vpositions.push(Cesium.SceneTransforms.wgs84ToWindowCoordinates($this._viewer.scene,positions[i]))
        // }
        // console.log('positions:', positions)
        // console.log('vpositions', vpositions)
        // console.log('mpositions',mpositions)
        voronoi1 = new voronoi({
          viewer: $this._viewer,
        }).calculateSurfaceArea(vpositions);
        // mianji1 = new mianji({
        //   viewer: $this._viewer,
        // }).calculateSurfaceAreaNew(mpositions)
        // console.log('mianji1',mianji1)
        console.log('voronoi1', voronoi1/1000000)
        turfmianji(positions)
        ...
        if (typeof options.callback === "function") {
          options.callback(
            ...
            //   ) / 1000000.0
            // ).toFixed(4)
            (turfarea/1000000).toFixed(4)
          );
        }
      }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);

      function turfmianji(position){
        let arr = []
        let a = $this.transformCartesianArrayToWGS84Array(position)
      ...
      function create() {
        ...
        polyObj = $this.objArr[objId].entities.add(_polygonEntity);
      }
      ...
      function _addInfoPoint(position) {
        ...
        _labelEntity.label = {
          text:
            ...
            (turfarea/1000000).toFixed(4)
            + "平方公里" + '\n' + 
            '占地面积:' + Number(voronoi1 / 1000000).toFixed(4) + '平方公里',
          show: true,
          showBackground: true,
          font: "14px monospace",
          horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
  ...
  drawTrianglesMeasureGraphics(options = {}) {
    ...
    if (this._viewer && options) {
      ...
      $this.objArr[objId].entities.add(_tempLineEntity2);
      $this.objArr[objId].entities.add(_tempLineEntity);
      $this.objArr[objId].entities.add(_trianglesEntity);
    }
  }
  ...
  show(objId, code) {
    ...
    if ($this.objArr.hasOwnProperty(objId)) {
      $this.objArr[objId].show = code;
    }
  }
  ...
  deleteByObjID(objId) {
    ...
    if ($this.objArr.hasOwnProperty(objId)) {
      console.log("You");
      $this.objArr[objId].entities.removeAll();
      delete $this.objArr[objId];
    } else {
      console.log("没有");
    }
  }
  ...
  flyToByObjID(objId) {
    var $this = this;
    if ($this.objArr.hasOwnProperty(objId)) {
      let target = $this.objArr[objId].entities.values[0].id;
      for (
        let index = 0;
        index < $this.objArr[objId].entities.values.length;
        index++
      ) {
        if (
          $this.objArr[objId].entities.values.polygon ||
          $this.objArr[objId].entities.values.polyline
        ) {
          target = $this.objArr[objId].entities.values[index].id;
          break;
        }
      }
      console.log(target);
      console.log($this.objArr[objId].entities.getById(target));
      let code = $this._viewer.flyTo(
        $this.objArr[objId].entities.getById(target),
        {
          offset: {
  ...
  get_Handler(viewer) {
    ...
    for (let index = 0; index < HandlerArr.length; index++) {
      HandlerArr[index].destroy();
